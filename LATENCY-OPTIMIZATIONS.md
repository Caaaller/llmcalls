...
| 13  | Brevity rule (`reason` ≤10 words) in user-message tail                                         | `2271e0c` 2026-04-23                         | Replay improved to 9/10 pass (brevity made Claude match GPT decisions more closely). Live latency: pending re-measurement — first run's `2908ms median` was **a false win** (see ❌ #3 below). |
